...
- ✅ 阶段 1 数据模型（见 §4）：12 张业务表、索引与约束、初始迁移与迁移执行脚本
- ✅ 阶段 2 内容渲染管线（见 §4）：唯一权威管线、Shiki 高亮、MathJax、
  表情、外链 rel、AST 层 @提及、长度与超时保护、预览端点、安全回归测试
- ✅ 阶段 3 站点域 + 公开 API 骨架（见 §4）：sites 三件套与配置 schema、
  site key 签发与轮换、来源匹配、CORS、错误信封、site:create CLI、GET /api/v1/config
- ✅ 阶段 4 评论读写核心（见 §4）：邮箱归并、线程物化计数、发表/回复/列表/
  回复分页/批量计数/最近评论/线程元信息/限流、公开响应白名单
- ✅ 阶段 5 审核、声誉与审计（见 §4）：状态机、mark-spam 原子副作用、
  邮箱声誉对称回滚、批量操作（单事务 + 部分失败明细）、原文编辑、审计日志、ILIKE 检索
- ✅ 阶段 6 邮件通知与 outbox worker（见 §4）：站点级 SMTP（密码加密）、模板、
...
- [x] 空库执行迁移成功，重复执行幂等
- [x] 表结构、约束、索引与 `requirements.md` §5.2 逐项一致
- [x] 迁移失败时不影响已启动的应用（因为它本来就不在启动流程里）
...
- [x] 预置的 XSS payload 全部被消毒，且合法 Markdown 不被误伤
- [x] 预览端点的输出与「落库时生成的 HTML」**逐字节一致**
      （结构性保证：两处调用同一个 `renderMarkdown`，并有纯函数测试守住）
- [x] 代码高亮、公式、表情均产出预期 HTML
- [x] 渲染耗时满足 §7.2 目标（单条 10KB < 80ms）
...
- [x] 一个**独立来源**的页面能跨域调用成功（用 curl 模拟第三方 Origin）
- [x] 非白名单 Origin 被拒；无来源头时按站点 `originPolicy` 分别表现 403 / 放行
- [x] `PUT` 只读端点返回 **405 + `Allow`**
- [x] **在生产构建下**验证 CORS 预检返回正确的 `Access-Control-Allow-Origin`
      （tests/http/ 会拉起生产产物跑，不依赖 `vite dev`）
...
- [x] 端到端：发表 → 列表 → 回复 → 计数正确
- [x] 公开响应断言**不含** `email` / `ip` / `user_agent`
- [x] **跨站点隔离测试**：用 A 站 site key 读写 B 站数据必须失败
- [x] 回复分页在热门评论下正常工作（顶层分页与回复分页互不干扰）
- [x] 邮箱为必填，缺失返回校验错误而非降级处理
...
- [x] 标记垃圾后，该邮箱在本站点的**下一条**评论自动进入 `pending`
- [x] 恢复为 approved 后，声誉计数与线程计数**对称回滚**
- [x] 批量操作部分失败时返回明细，且不留下部分写入
- [x] 关键词检索匹配的是**原文**，不是渲染后的 HTML
...
- [x] **SMTP 不可用时评论发布仍然成功**（异步化生效）
- [x] 失败任务记录错误并可按指数退避重试，超限后可在后台重发
      （重发入口 `retryOutboxItem` 已就绪，后台按钮在阶段 8）
- [x] 退订链接生效，且退订后不再收到该站点的邮件
- [x] 单条邮件投递失败不阻断同批次其他邮件
...
- [x] 无匹配角色的用户登录被拒，且**不产生会话**
- [x] `ADMIN.<siteA>` 角色的账号无法访问 siteB 的数据
- [x] 脚本通过 client credentials 取得 token 后可调用管理 API
      （Bearer 路径每次验签现算角色）
- [x] 会话可被吊销，吊销后立即失效
- [x] **不回显 `oidc_subject` 等内部标识给非管理员**
